        ...
        for file_name in os.listdir(path):
            ...
            if os.path.isfile(file_path):
                with open(file_path, 'r', encoding='utf8') as in_f:
                    lines = [line.strip().replace('BlockHttpDNS', 'reject').replace('REJECT', 'reject').replace('ChinaMax', 'direct').replace('Lan', 'direct').replace(' ', '').replace('Proxy', 'proxy').replace('GlobalMedia', 'proxy').replace('Global', 'proxy') for line in in_f.readlines() if not line.startswith("#")]
